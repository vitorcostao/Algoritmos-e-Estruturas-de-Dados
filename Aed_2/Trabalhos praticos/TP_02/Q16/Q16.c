...
Show LerCsv(char *title)
{
    ...
    // Definir dados
    Show resultado;
    FILE *csv = fopen("C:\\Users\\vitor\\OneDrive\\Documentos\\Aed_2\\Trabalhos praticos\\TP_02\\Q16\\disneyplus.csv", "rt");
...
int main()
{
    ...
    FILE *logFile = fopen("1521300_quicksort.txt", "w");
    if (logFile) {
      
        fprintf(logFile, "1521300\t%d\t%f\n", comparacoes, tempo_execucao);
        fclose(logFile);
    }
