...
class Show {
    ...
    /**
     * Algoritmo de inserção para ordenar Shows
     * 
     * @param array Arranjo de shows
     * @param comp  Quantia de comparações
     * 
     * @return Quantia de comparações
     */
    public static int InsertionSort(Show[] array, int comp) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            // Comparar e mover elementos maiores que tmp para frente
            while (j >= 0 && (array[j].getType().compareToIgnoreCase(tmp.getType()) > 0 ||
                  (array[j].getType().compareToIgnoreCase(tmp.getType()) == 0 &&
                   array[j].getTitle().compareToIgnoreCase(tmp.getTitle()) > 0))) {
...
public class Q07 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Ordenação
        comp = Show.InsertionSort(shows, comp);
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter("1521300_insercao.txt"))) {
