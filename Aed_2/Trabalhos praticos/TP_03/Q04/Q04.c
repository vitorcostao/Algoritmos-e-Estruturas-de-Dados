...
void InserirFilaCircular(Show show) {
    ...
    if(n < 5){
        ...
        for(int i = 0; i < 4; i++){
            ...
            FilaCircular[i] = FilaCircular[i + 1];
        }
...
float calcularMedia() {
    ...
    for(int i = 0; i < n; i++){
        ...
        if(FilaCircular[i].realeseYear > 0){

            soma += FilaCircular[i].realeseYear;
            count++;
        }
    }
...
int main() {
    ...
    for (int k = 0; k < x; k++) {
        ...
        if (strcmp(comando, "I") == 0) {
            ...
            char* id = strtok(NULL, " ");
            InserirFilaCircular(GetFileShow(id));
            media = calcularMedia(FilaCircular);
            printf("[Media] %d\n", media);
        } else if (strcmp(comando, "R") == 0) {

           removidos[j++] = RetirarFilaCircular(FilaCircular);
        }
    }

    ImprimirFila(FilaCircular);
