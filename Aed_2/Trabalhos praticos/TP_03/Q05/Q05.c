...
Lista* NovaLista() {
    ...
    lista->primeiro = NovaCelula((Show){});
    lista->ultimo = lista->primeiro;

    return lista;
}

int getListaTamanho(Lista* lista) {
...
void Inserir(Lista* lista, Show elemento, int pos) {
    ...
    if(pos < 0 || pos > getListaTamanho(lista)){

        printf("%d", getListaTamanho(lista));
...
Show RemoverInicio(Lista* lista) {
    ...
    // Definir dados
    Celula* removida = lista->primeiro->prox;
    Show elemento = removida->elemento;
    lista->primeiro->prox = removida->prox;

    if(lista->ultimo == removida){
...
Show RemoverFim(Lista* lista) {
    ...
    // Definir dados
    Celula* atual = lista->primeiro;
    while(atual->prox != lista->ultimo && atual->prox != NULL){
...
void LerCsvId(Lista* lista, char* idStr) {
    ...
    while (fgets(buffer, sizeof(buffer), csv) && flag == 0) {
        ...
        if( idLinha == idAlvo) {
            Show show = Ler(buffer);
            quickSort(0, show.listedInCount - 1, show.listedIn);
            quickSort(0, show.castCount - 1, show.cast);
...
int main() {
    ...
    for (int k = 0; k < x; k++) {
        ...
        if (strcmp(comando, "II") == 0) {
            char* id = strtok(NULL, " ");
            InserirInicio(lista, GetFileShow(id));

        } else if (strcmp(comando, "IF") == 0) {
            char* id = strtok(NULL, " ");
            InserirFim(lista, GetFileShow(id));

        } else if (strcmp(comando, "I*") == 0) {
            char* posStr = strtok(NULL, " ");
            char* id = strtok(NULL, " ");
            int pos = atoi(posStr);
            Inserir(lista, GetFileShow(id), pos);

        } else if (strcmp(comando, "RI") == 0) {
            removidos[j++] = RemoverInicio(lista);
            

        } else if (strcmp(comando, "RF") == 0) {
            removidos[j++] = RemoverFim(lista);

        } else if (strcmp(comando, "R*") == 0) {
            char* posStr = strtok(NULL, " ");
            int pos = atoi(posStr);
            removidos[j++] = Remover(lista, pos);
    ...
    int tam = getListaTamanho(lista);

    for (int i = 0; i < tam; i++) {
        Show show = lista->primeiro->prox->elemento;
        imprimirShow(show);
