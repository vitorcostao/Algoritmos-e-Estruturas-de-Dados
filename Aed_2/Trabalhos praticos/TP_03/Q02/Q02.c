...
int main() {
    ...
    for (int k = 0; k < x; k++) {
        ...
        if (strcmp(comando, "II") == 0) {
            char* id = strtok(NULL, " ");
            inserirInicio(GetFileShow(id));

        } else if (strcmp(comando, "IF") == 0) {
            char* id = strtok(NULL, " ");
            inserirFim(GetFileShow(id));

        } else if (strcmp(comando, "I*") == 0) {
            char* posStr = strtok(NULL, " ");
            char* id = strtok(NULL, " ");
            int pos = atoi(posStr);
            inserir(GetFileShow(id), pos);

        } else if (strcmp(comando, "RI") == 0) {
            removidos[j++] = removerInicio();
            

        } else if (strcmp(comando, "RF") == 0) {
            removidos[j++] = removerFim();

        } else if (strcmp(comando, "R*") == 0) {
            char* posStr = strtok(NULL, " ");
            int pos = atoi(posStr);
            removidos[j++] = remover(pos);
