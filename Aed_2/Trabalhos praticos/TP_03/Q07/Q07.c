...
void Enfilerar(Fila* fila, Show elemento) {
...
float CalcularMedia(Fila* fila) {
    ...
    do {
        soma += atual->elemento.realeseYear;
        atual = atual->prox;

    } while(atual != fila->primeiro);

    return (float)soma / fila->n;
}
 ...
 * @return Vetor de structs Show alocado dinamicamente.
 */
void LerCsvId(Fila* FilaCircular, char* idStr) {
...
int main() {
    ...
    // Definir dados
    Fila* FilaCircular = NovaFilaCircular();
    int x = 0;
    int media = 0;
    ...
    while (strcmp(input, "FIM") != 0) {
        LerCsvId(FilaCircular, input);
        media = CalcularMedia(FilaCircular);
        printf("[Media] %d\n", media);
        scanf(" %1000[^\n]", input);
    }
    ...
    for (int k = 0; k < x; k++) {
        ...
        if (strcmp(comando, "I") == 0) {
            ...
            char* id = strtok(NULL, " ");
            Enfilerar(FilaCircular, GetFileShow(id));
            media = CalcularMedia(FilaCircular);
            printf("[Media] %d\n", media);
        } else if (strcmp(comando, "R") == 0) {

           Desenfilerar(FilaCircular);
        }
    }
