        ...
        for(k = 0; k < n; k++){
            ...
            switch (partes[0]) {
                case "II" -> {
                    ...
                    shows.inserirInicio(Show.AddVetor("/tmp/disneyplus.csv", partes[1]));
                }       
                case "IF" -> {
                    shows.inserirFim(Show.AddVetor("/tmp/disneyplus.csv", partes[1]));
                }
                case "I*" -> {
                    shows.inserir(Show.AddVetor("/tmp/disneyplus.csv", partes[2]), Integer.parseInt(partes[1]));
                }
                case "RI" -> {
                    removidos[j] = shows.removerInicio();
                    j++;
                }
                case "RF" -> {
                    removidos[j] = shows.removerFim();
                    j++;
                }
                case "R*" -> {

                    
                    removidos[j] = shows.remover(Integer.parseInt(partes[1]));
                    j++;
                }
                default -> {
                }
